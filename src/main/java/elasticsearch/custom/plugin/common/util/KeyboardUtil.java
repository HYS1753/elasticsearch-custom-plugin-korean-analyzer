...
public class KeyboardUtil {
     ...
     * 영문 정규 표현식
     */
    public static final String ENG_REGEX = "[a-zA-Z]";

    /**
     * 초성 키에 해당하는 키보드상의 영문자 (19자)
     */
    public static final String[] KEYBOARD_CHO_SEONG = {
            "r", "R", "s", "e", "E", "f", "a", "q", "Q", "t", 
            "T", "d", "w", "W", "c", "z", "x", "v", "g"
    };
     ...
     */
    public static final String[] KEYBOARD_JUNG_SEONG = {
     ...
     */
    public static final String[] KEYBOARD_JONG_SEONG = {
     ...
     * @return
     */
    public static Map<String, Integer> getInfoForChoSeong(int index, String word) {
        int code = KeyboardUtil.makeUnicodeIndex(CodeType.CHOSEONG, word.substring(index, index + 1));
        int idx = index + 1;
     ...
     * @return
     */
    public static Map<String, Integer> getInfoForJungSeong(int index, String word) {
     ...
     * @return
     */
    public static Map<String, Integer> getInfoForJongSeong(int index, String word) {
        int code;
        int idx = index;
    ...
    private static int getSingleMedial(int index, String word) {
        if ((index + 1) <= word.length()) {
            return makeUnicodeIndex(CodeType.JUNGSEONG, word.substring(index, index+1));
        } else {
            return -1;
        }
    }
    ...
    private static int getDoubleMedial(int index, String word) {
        if ((index + 2) > word.length()) {
            ...
            return makeUnicodeIndex(CodeType.JUNGSEONG, word.substring(index, index+2));
        }
    }
    ...
    private static int getSingleFinal(int index, String word) {
        if ((index + 1) <= word.length())  {
            return makeUnicodeIndex(CodeType.JONGSEONG, word.substring(index, index+1));
        } else {
            return -1;
        }
    }
    ...
    private static int getDoubleFinal(int index, String word) {
        if ((index + 2) > word.length()) {
            ...
            return makeUnicodeIndex(CodeType.JONGSEONG, word.substring(index, index+2));
        }
    }
    ...
    private static int makeUnicodeIndex(CodeType type, String subStr) {
        switch (type) {
            case CHOSEONG:
                for (int i=0; i<KEYBOARD_CHO_SEONG.length; i++) {
                    if (KEYBOARD_CHO_SEONG[i].equals(subStr)) {
                        return i * 28 * 21; 
                    }
                }                
                break;
            ...
            case JUNGSEONG:
                for (int i=0; i<KEYBOARD_JUNG_SEONG.length; i++) {
                    if (KEYBOARD_JUNG_SEONG[i].equals(subStr)) {
                        return i * 28;
                    }
                }
                break;
            ...
            case JONGSEONG:
                for (int i=0; i<KEYBOARD_JONG_SEONG.length; i++) {
                    if (KEYBOARD_JONG_SEONG[i].equals(subStr)) {
                        return i + 1;
                    }
                }
                break;
